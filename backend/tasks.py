...
def init_worker(**kwargs):
    global openai_client, playwright_instance, browser_instance, product_database, content_map
...
def find_mentioned_products(text_content: str) -> list:
    if not product_database or not text_content:
        return []
    mentioned = []
...
def find_relevant_cluster(text_content: str) -> dict:
    if not content_map or not text_content:
        return None
...
def generate_preview_task(job_id: str, url: str):
    ...
    if not browser_instance:
        log_terminal(f"❌ Preview job {job_id} failed: Browser not available.")
        return
...
def run_project_task(self, job_id: str, project_data: dict):
    global browser_instance
    if not browser_instance:
    ...
    log_terminal(f"--- [RUNNER] Cleared old processed URLs for key: {processed_urls_key} ---")
    ...
    try:
        ...
        for item in article_links:
            ...
            # --- *** NEW, ROBUST DATE FILTERING LOGIC *** ---
            should_process = False # Default to not processing
            ...
            # If the article is not from today, update status and skip to the next one
