      ...
      <div className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl border border-gray-200">
          <h2 className="text-xl font-semibold text-gray-700 mb-4 flex items-center">
            ...
            Google Sheets Price Importer
          </h2>
          <p className="text-sm text-gray-500 mb-4">
            Import product pricing data from a Google Sheet. The sheet must be shared with your service account email.
          </p>
          <div className="border-t pt-4 space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Google Sheet URL</label>
              <input type="text" value={sheetUrl} onChange={(e) => setSheetUrl(e.target.value)} placeholder="Paste the full URL of your Google Sheet..." className="w-full p-2 border rounded-md"/>
            </div>
            <div className="text-right">
              <button onClick={handleImport} disabled={isImporting} className="inline-flex items-center px-6 py-2 bg-indigo-600 text-white font-semibold rounded-md shadow-sm hover:bg-indigo-700 disabled:bg-gray-400">
                {isImporting ? <Loader2 className="w-5 h-5 mr-2 animate-spin"/> : null}
                {isImporting ? 'Importing...' : 'Import & Update Prices'}
              </button>
            </div>
          </div>
        </div>
