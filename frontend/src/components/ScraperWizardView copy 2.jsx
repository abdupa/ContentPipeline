...
import { PlusCircle, Trash2, ArrowRight, Target, Loader2, Save, List, MousePointerClick } from 'lucide-react';
...
const Step2_LinkSelection = ({ urlInput, setUrlInput, onNext, onBack }) => {
    const [scrapeMode, setScrapeMode] = useState('interactive'); // 'interactive' or 'manual'
    
    // State for the interactive part
    ...
    const handleNext = () => {
        if (scrapeMode === 'manual') {
            if (!urlInput.trim()) {
                alert("Please enter at least one URL in the list.");
                return;
            }
        } else { // interactive
            if (capturedLinks.length === 0) {
                alert("Please select at least one link from the preview.");
                return;
            }
            // If interactive, we update the URL list with the captured links for the next step
    ...
    return (
        <div>
          ...
          <p className="text-gray-600 mt-1 mb-6">Choose how to provide the list of pages you want to scrape.</p>
          
          <div className="flex rounded-lg p-1 bg-gray-200 w-fit mb-6">
              <button onClick={() => setScrapeMode('interactive')} className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors flex items-center gap-2 ${scrapeMode === 'interactive' ? 'bg-white text-blue-600 shadow' : 'bg-transparent text-gray-600'}`}><MousePointerClick className="w-4 h-4" /> Interactive Selection</button>
              <button onClick={() => setScrapeMode('manual')} className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors flex items-center gap-2 ${scrapeMode === 'manual' ? 'bg-white text-blue-600 shadow' : 'bg-transparent text-gray-600'}`}><List className="w-4 h-4" /> Manual URL List</button>
          </div>

          {scrapeMode === 'manual' ? (
            <div>
                <label htmlFor="url-input" className="block text-sm font-medium text-gray-700 mb-1">URLs to Scrape (one per line)</label>
                <p className="text-xs text-gray-500 mb-2">The first URL in this list will be used as a model for the preview in the next step.</p>
                <textarea id="url-input" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} placeholder="https://www.example.com/page-1&#10;https://www.example.com/page-2" className="w-full h-64 p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 font-mono text-sm"/>
            </div>
          ) : (
            <div>
              <p className="text-gray-600 mt-1 mb-4">Enter a starting URL below, fetch the preview, then click on the links that lead to the pages you want to process.</p>
              <div className="flex gap-4 mb-4">
                <input type="text" value={urlInput.split('\n')[0] || ''} onChange={e => setUrlInput(e.target.value)} placeholder="Enter a starting URL to preview..." className="w-full p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500" />
...
const Step3_FieldsAndPrompts = ({ 
    ...
    // ... (This component's internal logic is the same as the previous corrected version)
    const [previewHtml, setPreviewHtml] = useState("");
    const [isLoading, setIsLoading] = useState(false);
    const [previewJobId, setPreviewJobId] = useState(null);
    const intervalRef = useRef(null);
