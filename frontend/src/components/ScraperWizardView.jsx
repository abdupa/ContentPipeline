...
import { PlusCircle, Trash2, ArrowRight, Target, Loader2, Save } from 'lucide-react';
...
// --- Child components (Step1, Step2, Step3) remain unchanged from before ---
// (For brevity, their code is omitted here but is the same as the previous version)
const Step1_ProjectDetails = ({ projectName, setProjectName, projectType, setProjectType, urlInput, setUrlInput, onNext }) => {
    return (
    <div>
      <h2 className="text-2xl font-bold text-gray-800">Step 1: Project Details & URLs</h2>
      <p className="text-gray-600 mt-1 mb-6">Give your project a name and define the initial URLs to scrape.</p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div>
          ...
          <select id="project-type" value={projectType} onChange={(e) => setProjectType(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
            <option value="News">News</option><option value="Specs">Specs</option><option value="Review">Review</option>
          </select>
        </div>
      </div>
      <div>
        <label htmlFor="url-input" className="block text-sm font-medium text-gray-700 mb-1">Initial URLs (one per line)</label>
        <textarea id="url-input" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} placeholder="https://www.gsmarena.com/news.php3" className="w-full h-40 p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 font-mono text-sm"/>
      </div>
      <div className="mt-8 text-right">
        <button type="button" onClick={onNext} disabled={!urlInput.trim() || !projectName.trim()} className="px-6 py-2 bg-indigo-600 text-white font-semibold rounded-md shadow-sm hover:bg-indigo-700 disabled:bg-gray-300 flex items-center gap-2 ml-auto">Next: Define Links <ArrowRight className="w-5 h-5" /></button>
      </div>
    </div>
  );
};
const Step2_LinkSelection = ({ urls, scrapeMode, setScrapeMode, crawlingLevels, setCrawlingLevels, capturedLinks, setCapturedLinks, finalCrawlSelections, setFinalCrawlSelections, setSelectedModelUrl, onNext, onBack }) => {
    ...
    const handleFetchPreview = async () => {
        ...
        if (scrapeMode === 'direct' || activeCrawlIndex === 0) {
            ...
            if (!prevLevel || !prevLevel.previewUrl) { alert("Please define a link for the previous level first to load the next preview."); return; }
        ...
        try {
            ...
            setIsLoading(false); setPreviewHtml(`<div class='p-4 text-red-600'>Error starting preview job. Check console for details.</div>`);
        }
    };
    useEffect(() => {
        ...
        const pollForResult = async () => {
            try {
                ...
                else if (status === 'failed') { setPreviewHtml(`<div class='p-4 text-red-600'><b>Preview generation failed.</b><br/>Error: ${error}</div>`); setIsLoading(false); stopPolling(); }
            } catch (err) { console.error(`Error while polling for job ${previewJobId}:`, err); setIsLoading(false); stopPolling(); }
        };
        pollForResult(); intervalRef.current = setInterval(pollForResult, 2500);
        return stopPolling;
    }, [previewJobId]);
    useEffect(() => {
        const handleMessage = (event) => {
            if (event.data && event.data.type === 'selection-updated') {
                ...
                if (scrapeMode === 'direct') { setCapturedLinks(elements); }
                else if (isFinalCrawlLevel) {
                    setFinalCrawlSelections(elements);
                    const lastElement = elements[elements.length - 1];
                    if (lastElement) { setCrawlingLevels(prev => prev.map((l, i) => i === activeCrawlIndex ? { ...l, selector: lastElement.selector } : l)); }
                } else {
    ...
    const handleNext = () => {
        let urlForStep3 = null; let baseForStep3 = null;
        if (scrapeMode === 'direct') {
            if (capturedLinks.length === 0) { alert("Please select at least one link."); return; }
            urlForStep3 = capturedLinks[0].href; baseForStep3 = urls[0];
        } else if (scrapeMode === 'crawl') {
            const intermediateLevels = crawlingLevels.slice(0, -1);
            if (intermediateLevels.some(l => !l.selector)) { alert("Please define a link for all intermediate crawl levels first."); return; }
            if (finalCrawlSelections.length === 0) { alert("Please select at least one item on the final crawl level."); return; }
            urlForStep3 = finalCrawlSelections[0].href; baseForStep3 = currentPreviewBaseUrl;
        } else { return; }
        if (urlForStep3 && baseForStep3) {
            const absoluteUrl = new URL(urlForStep3, baseForStep3).href;
            setSelectedModelUrl(absoluteUrl); onNext();
        } else { alert("Could not determine a valid URL for the next step."); }
    };
        ...
        <div>
          ...
          {scrapeMode === 'direct' ? (
            <div>
              ...
              <div className="mb-4"><button type="button" onClick={handleFetchPreview} disabled={isLoading} className="px-4 py-2 bg-blue-600 text-white font-semibold rounded-md shadow-sm hover:bg-blue-700 disabled:bg-gray-400">{isLoading ? <span className="flex items-center"><Loader2 className="animate-spin mr-2" /> Generating Preview...</span> : "Fetch Live Preview"}</button></div>
              <div className="border-2 border-gray-300 rounded-lg bg-gray-100 mb-6"><div className="w-full h-96 bg-white"><iframe srcDoc={previewHtml || "<p class='p-4 text-gray-500'>Click 'Fetch Live Preview'</p>"} title="Live Page Preview" className="w-full h-full" sandbox="allow-scripts allow-same-origin" /></div></div>
              <div className="mt-6">
            ...
            <div>
              ...
              {isFinalCrawlLevel && (<div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg"><h3 className="text-lg font-medium text-blue-800">Final Item Selection</h3><p className="text-blue-700">You are on the final level. Click all the items you want to scrape from the preview.</p><p className="text-lg font-bold text-blue-900 mt-2">{finalCrawlSelections.length} items selected.</p></div>)}
            </div>
          )}
...
const Step3_ElementsAndPrompt = ({ selectedModelUrl, elementRules, setElementRules, llmPrompt, setLlmPrompt, onBack, onSaveProject }) => {
    const [previewHtml, setPreviewHtml] = useState("");
    const [isLoading, setIsLoading] = useState(false);
    const [previewJobId, setPreviewJobId] = useState(null);
    const intervalRef = useRef(null);
    ...
    useEffect(() => {
        const fetchPreview = async () => {
            ...
            try {
                ...
                setPreviewJobId(response.data.job_id);
            } catch (error) { console.error("Error starting Step 3 preview job:", error); setIsLoading(false); setPreviewHtml(`<div class='p-4 text-red-600'>Error starting preview job.</div>`); }
        };
    ...
    return (
        <div>
          ...
          <p className="text-gray-600 mt-1 mb-6">Click elements in the preview to extract them. Then, write a prompt for the AI using the field names as placeholders.</p>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              <div>
                ...
                <p className="text-sm text-gray-500 mb-2">Click in the preview to add rows. Edit field names to be simple, one-word placeholders (e.g., 'title', 'summary', 'date').</p>
                <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 bg-white max-h-60">
              ...
              <div className="mt-6">
                ...
                <p className="text-sm text-gray-500 mb-2">Use your field names in curly braces as placeholders, e.g., `{'{title}'}`.</p>
                <textarea id="llm-prompt" value={llmPrompt} onChange={(e) => setLlmPrompt(e.target.value)} placeholder="Based on the article with title '{title}' and summary '{summary}', write a 500-word blog post..." className="w-full h-48 p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 font-mono text-sm"/>
              </div>
            </div>
          </div>
          <div className="mt-8 flex justify-between">
            <button type="button" onClick={onBack} className="px-6 py-2 bg-gray-200 text-gray-800 font-semibold rounded-md hover:bg-gray-300">Back</button>
            <button type="button" onClick={onSaveProject} disabled={elementRules.length === 0 || !llmPrompt.trim()} className="px-8 py-3 bg-green-600 text-white font-bold text-lg rounded-full shadow-lg hover:bg-green-700 disabled:bg-gray-400 flex items-center gap-2"><Save className="w-5 h-5" /> Save Project</button>
          </div>
        </div>
    );
};
...
const ScraperWizardView = ({ onProjectSaved, projectToEdit }) => {
  ...
  // State for all project configuration data
  const [projectName, setProjectName] = useState('');
  const [projectType, setProjectType] = useState('News');
  ...
  // NEW: Effect to pre-populate the wizard when editing a project
  useEffect(() => {
    if (projectToEdit) {
      ...
      setScrapeMode(config.scrape_type);
      setElementRules(config.element_rules.map(rule => ({ ...rule, value: '' }))); // value is transient
      
      // Note: Re-hydrating crawl/link selections is complex and often not needed for editing.
      // The user will typically re-select these if they need to change them.
      // We reset them here for a clean edit session.
      setCrawlingLevels(config.crawling_levels?.length ? config.crawling_levels : [{ name: 'Level 1', selector: '', previewUrl: null }]);
      setCapturedLinks([]);
      setFinalCrawlSelections([]);

    } else {
      // If not editing, reset all fields to their initial state
      resetWizardState();
    }
  }, [projectToEdit]);
  ...
  const handleSaveProject = async () => {
    const scrapeConfig = {
      ...
      crawling_levels: scrapeMode === 'crawl' ? crawlingLevels.slice(0, -1).map(({ name, selector }) => ({ name, selector })) : [],
      final_urls: scrapeMode === 'crawl' ? [...new Set(finalCrawlSelections.map(link => new URL(link.href, selectedModelUrl).href))] : [],
      modelUrls: scrapeMode === 'direct' ? [...new Set(capturedLinks.map(link => new URL(link.href, urlInput.split('\n')[0]).href))] : [],
      element_rules: elementRules.map(({ name, selector }) => ({ name, selector })),
    };

    const projectData = {
      // If we are editing, use the existing ID to overwrite the project
    ...
    try {
      // The backend will create a new project or update an existing one based on the presence of project_id
  ...
  const renderStep = () => {
    ...
    switch (currentStep) {
      case 1:
        return <Step1_ProjectDetails projectName={projectName} setProjectName={setProjectName} projectType={projectType} setProjectType={setProjectType} urlInput={urlInput} setUrlInput={setUrlInput} onNext={() => setCurrentStep(2)} />;
      case 2:
        return <Step2_LinkSelection urls={urls} scrapeMode={scrapeMode} setScrapeMode={setScrapeMode} crawlingLevels={crawlingLevels} setCrawlingLevels={setCrawlingLevels} capturedLinks={capturedLinks} setCapturedLinks={setCapturedLinks} finalCrawlSelections={finalCrawlSelections} setFinalCrawlSelections={setFinalCrawlSelections} setSelectedModelUrl={setSelectedModelUrl} onNext={() => setCurrentStep(3)} onBack={() => setCurrentStep(1)} />;
      case 3:
        return <Step3_ElementsAndPrompt selectedModelUrl={selectedModelUrl} elementRules={elementRules} setElementRules={setElementRules} llmPrompt={llmPrompt} setLlmPrompt={setLlmPrompt} onBack={() => setCurrentStep(2)} onSaveProject={handleSaveProject} />;
      default:
    ...
    <div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl w-full max-w-6xl border border-gray-200">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center text-sm font-semibold text-gray-500">
          ...
          <span className={currentStep >= 2 ? 'text-indigo-600' : ''}>Step 2: Links</span>
